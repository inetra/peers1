...
CPictureExWnd::CPictureExWnd()
{
	...
	downloader = new HttpConnection(PeersUtils::getUserAgent());
...
void CPictureExWnd::freeBannerData()
{
	if (bannerDataLength > 0 && bannerData) 
	{
		GlobalFree(bannerData);
		bannerDataLength = 0;
		bannerData = 0;
	}
}
...
LRESULT CPictureExWnd::OnLButtonUp(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) 
{
	if (!banner_cgihref.empty())
	{
		downloader->followRedirection = false;
		downloader->downloadFile(banner_cgihref);
	}
	return S_OK;
}

LRESULT CPictureExWnd::OnSetCursor(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) 
{
	if (!banner_cgihref.empty() && m_downloadStage == CLICK_STAGE)
	{
		::SetCursor(m_Cursor);
	}
	return S_OK;
...
void CPictureExWnd::LoadAdRiverXMLBanner(const string& sid, const string& sz, const string& bn, const string& bt, const string& pz)
{
	stringstream url;
	url << "http://ad.adriver.ru/cgi-bin/erle.cgi";
	url << "?sid=" << sid;
	url << "&sz=" << sz;
	url << "&bn=" << bn;
	url << "&bt=" << bt;
	...
	Lock l(cs);
	m_downloaderData.clear();
	banner_cgihref.clear();
	this->src = url.str();
	m_downloadStage = FIRST_STAGE;
	downloader->followRedirection = true;
	downloader->downloadFile(src);
}

void CPictureExWnd::UnLoadAdRiverXMLBanner() {
	Lock l(cs);
	UnLoad();
	freeBannerData();
	m_downloadStage = UNKNOWN_STAGE;
	downloader->cookies.clear();
	downloader->cancelDownload();
}


void CPictureExWnd::on(HttpConnectionListener::Data, HttpConnection *, const uint8_t *buf, size_t len) throw()
{
	m_downloaderData += string((const char*)buf, len);
}

void CPictureExWnd::on(HttpConnectionListener::Complete, HttpConnection* /*aConn*/, const string&) throw()
{
	Lock l(cs);
	SimpleXML xml;
	string tmp, banner_comppath;
	char *pData;
	switch (m_downloadStage) 
	{
		case FIRST_STAGE:
			try 
			{
				xml.fromXML(m_downloaderData);
				xml.stepIn();
				while(xml.findChild("item"))
				{
					xml.stepIn();
					banner_zero_comppath = xml.getChildData("ar_zero_comppath");
					banner_comppath = xml.getChildData("ar_comppath");
					ar_bn = xml.getChildData("ar_bn");
					ar_adid = xml.getChildData("ar_adid");
					ar_bid = xml.getChildData("ar_bid");
					ar_sid = xml.getChildData("ar_sid");
					ar_bt = xml.getChildData("ar_type");
					ar_sz = xml.getChildData("ar_sz");
					ar_pz = xml.getChildData("ar_pz");
					ar_rnd = xml.getChildData("ar_rnd");
					banner_cgihref = "http://ad.adriver.ru/cgi-bin/click.cgi"
						 "?bn=" + ar_bn +
						 "&ad=" + ar_adid +
						"&bid=" + ar_bid +
						"&sid=" + ar_sid +
						...
						"&rnd=" + ar_rnd +
						"&ref=&custom=";
					tmp = xml.getChildData("ar_name");
					xml.stepOut();
				}
				if (!banner_comppath.empty() && !tmp.empty()) {
					src = banner_comppath + tmp;
					m_downloadStage = SECOND_STAGE;
					m_downloaderData.clear();
					downloader->downloadFile(src);
				}
		...
		case SECOND_STAGE:
			try 
			{
				xml.fromXML(m_downloaderData);
				xml.stepIn();
				while (xml.findChild("content_image")) 
				{
					xml.stepIn();
					tmp = xml.getChildData("localUrl");
					xml.stepOut();
				}
				if (!banner_zero_comppath.empty()&& !tmp.empty()) 
				{
					src = banner_zero_comppath + tmp;
					m_downloadStage = IMAGE_STAGE;
					m_downloaderData.clear();
					downloader->downloadFile(src);
				}
			} catch (const Exception& e) {
			...
			break;
		case IMAGE_STAGE:
			bannerDataLength = m_downloaderData.length();
			bannerData = GlobalAlloc(GMEM_MOVEABLE | GMEM_NODISCARD, bannerDataLength);
			
			if (!bannerData)
			{
				ATLTRACE(_T("Load (http): Error allocating memory\n"));
				bannerDataLength = 0;
				return;
			};
	
			pData = reinterpret_cast<char*>(GlobalLock(bannerData));
			if (!pData)
			{
				ATLTRACE(_T("Load (http): Error locking memory\n"));
				GlobalFree(bannerData);
				bannerData = 0;
				bannerDataLength = 0;
				return;
			...
			memcpy(pData, m_downloaderData.data(), bannerDataLength);

			GlobalUnlock(bannerData);
