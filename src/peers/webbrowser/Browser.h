...
public:
	...
	LRESULT OnDestroy(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
	{
        ...
        HRESULT createControl() {
          ...
          HRESULT hr = pT->QueryHost(&spWinHost);
          // If QueryHost failed, there is no host attached to this window
          if (SUCCEEDED(hr)) {
            // Create the WebBrowser control
            WCHAR htmlCLSID[64];
            StringFromGUID2(__uuidof(WebBrowser), htmlCLSID, sizeof(htmlCLSID));
            hr = spWinHost->CreateControl(htmlCLSID, pT->m_hWnd, NULL);
            if (SUCCEEDED(hr)) {
              CComPtr<IUnknown> pControl;
              hr = spWinHost->QueryControl(__uuidof(IUnknown), (void**) &pControl);
              if (SUCCEEDED(hr)) {
                hr = DispEventAdvise(pControl, &DIID_DWebBrowserEvents2);
                if (SUCCEEDED(hr)) {
                  m_pBrowser = pControl;
                }
              }
	...
	*/
	...
	HRESULT Navigate(LPCTSTR szURL,DWORD dwFlags=0,LPCTSTR szTargetFrameName=NULL,LPCVOID pPostData=NULL,DWORD dwPostDataLength=0,LPCTSTR szHeaders=NULL)
	{
		...
		if (szHeaders)
			vtHeaders=szHeaders;
                CComVariant vtFlags((LONG)dwFlags);
		return m_pBrowser->Navigate(T2BSTR(szURL),&vtFlags,&vtTargetFrameName,&vtPostData,&vtHeaders);
	}
...
private:
	...
	void __stdcall __NavigateError(IDispatch* pDisp, VARIANT* pvURL, VARIANT* pvTargetFrameName, VARIANT* pvStatusCode, VARIANT_BOOL* pbCancel)
	{
		...
		ATLASSERT(pbCancel != NULL);
                *pbCancel=(VARIANT_BOOL) pT->OnNavigateError(pDisp,V_BSTR(pvURL),V_BSTR(pvTargetFrameName),V_I4(pvStatusCode));
	}
