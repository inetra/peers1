...
inline BOOL IsBadStringPtr(LPCTSTR lpsz, int nLength = -1)
{
	if (lpsz == NULL)
		return TRUE;
...
{
public:
   ATL::CSimpleArray<CIdleHandlerEx*> m_aIdleHandler;
...
class CTransparentWindow : public ATL::CWindow
{
public:
   CTransparentWindow(HWND hWnd = NULL) : ATL::CWindow(hWnd)
   {
   }
...
class CAnimateWindow : public ATL::CWindow
{
public:
   CAnimateWindow(HWND hWnd = NULL) : ATL::CWindow(hWnd)
   {
   }
...
struct CLockStaticDataInit
{
	...
	CLockStaticDataInit() : m_cslock(ATL::_pAtlModule->m_csStaticDataInitAndTypeInfo, true) { };
#else
...
class CLangString : public _CSTRING_NS::CString
{
public:
#ifndef CHAR_FUDGE
   #ifdef _UNICODE
...
#endif // CHAR_FUDGE
   CLangString() { };
   CLangString(const _CSTRING_NS::CString& s) : _CSTRING_NS::CString(s) { };
   CLangString(const CLangString& s) : _CSTRING_NS::CString(s) { };
   CLangString(LPCTSTR pstr) : _CSTRING_NS::CString(pstr) { };
   ...
   BOOL LoadString(WORD wLanguage, UINT nID)
   {
      ...
      if( nCount - nLen > CHAR_FUDGE ) {
         * (_CSTRING_NS::CString*) this = szTemp;
         return nLen > 0;
      }
   ...
   static int __stdcall _LoadString(WORD wLanguage, UINT nID, LPTSTR lpszBuf, UINT nMaxBuf)
   {
      ...
      // OPTI: Should use _CSTRING_NS::CString::_wcstombsz instead.
...
public:
   ...
   {
      ATLASSERT(!::IsBadStringPtr(pstrSrc, UINT_PTR(-1)));
      if( m_pstr ) delete [] m_pstr;
      int len = _tcslen(pstrSrc);
      ATLTRY(m_pstr = new TCHAR[len+1]);
      ATLASSERT(m_pstr);
      if( m_pstr == NULL ) return 0;
      _tcscpy(m_pstr, pstrSrc);
      return len;
   }
};
...
public:
...
#if defined(UNDER_CE)
   FARPROC GetProcAddress(LPCWSTR pszFuncName) const
   {
      ATLASSERT(!::IsBadStringPtr(pszFuncName, UINT_PTR(-1)));
      ATLASSERT(IsLoaded()); 
   ...
   {
      ATLASSERT(!::IsBadStringPtrA(pszFuncName, UINT_PTR(-1)));
      ATLASSERT(IsLoaded()); 
      return ::GetProcAddress(m_hInst, pszFuncName);
   }
#endif	// UNDER_CE
...
/////////////////////////////////////////////////////////////////////////////
// ATL::CWindowPlacement

#if !defined(_WTL_NO_WTYPES) && !defined(UNDER_CE)
...
public:
   ...
   {
      ATLASSERT(!::IsBadStringPtr(pstr, UINT_PTR(-1)));
      if( _tcslen(pstr) == 0 ) return FALSE;
      flags = _GetInt(pstr);
   ...
   {
      ATLASSERT(hReg);
      ATLASSERT(!::IsBadStringPtr(pstrKeyName, UINT_PTR(-1)));
      TCHAR szData[80];
   ...
   {
      ATLASSERT(!::IsBadStringPtr(pstrValueName, UINT_PTR(-1)));
      ATLASSERT(hReg);
      TCHAR szData[80];
      if( !SetPosData(szData, (UINT) sizeof(szData)/sizeof(TCHAR)) ) return FALSE;
      return ::RegSetValueEx(hReg, pstrValueName, NULL, REG_SZ, (CONST BYTE*) szData, (_tcslen(szData)+1)*sizeof(TCHAR)) == ERROR_SUCCESS;
   }
...
public:
   ...
   static BOOL CALLBACK _FindProc(HWND /*hWnd*/, LPTSTR pstrName, HANDLE /*hData*/, ULONG_PTR lParam)
   {
      ...
      if( (HIWORD(pstrName) == 0 && pstrName == pf->pstrName) ||
          (HIWORD(pstrName) != 0 && _tcscmp(pstrName, pf->pstrName) == 0) ) 
      {
         pf->bFound = TRUE;
         return FALSE;
      }
...
public:
   ...
   BOOL GetString(LPCTSTR pstrSection, LPCTSTR pstrKey, LPTSTR pstrValue, UINT cchMax, LPCTSTR pstrDefault = NULL) const
   {
      ...
      ATLASSERT(!::IsBadStringPtr(pstrSection, UINT_PTR(-1)));
      ATLASSERT(!::IsBadStringPtr(pstrKey, UINT_PTR(-1)));
...
#if defined(__ATLSTR_H__) || defined(_WTL_USE_CSTRING)
   BOOL GetString(LPCTSTR pstrSection, LPCTSTR pstrKey, _CSTRING_NS::CString& sValue, LPCTSTR pstrDefault = NULL) const
   {      
...
inline _CSTRING_NS::CString AtlLoadHtmlResource(ATL::_U_STRINGorID html)
{
#if (_ATL_VER >= 0x0700)
