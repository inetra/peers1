...
/* проверка подозрительно выгл€дещего размера шары */
...
/* нотификаци€ о том, что у пользовател€ размер шары подозрительный */
...
LRESULT HubFrame::onSpeaker(UINT /*uMsg*/, WPARAM /* wParam */, LPARAM /* lParam */, BOOL& /*bHandled*/) {
  ...
  for(TaskQueue::Iter i = t.begin(); i != t.end(); ++i) {
	  if (i->first == SUBSCRIPTIONS) {
		  ...
		  {
			  // второе условие - информаци€ о версии хаба (первый пользователь наш, второй - хаб).
#ifdef _DEBUG
...
bool HubFrame::isWrongNick() {
	static const tstring patterns[] = {
		_T("Ќикнейм зан€т другим пользователем"),
		_T("You are already in the hub"),
		_T("You have already logged into"),
		_T("ѕользователь с таким ником уже зарегистрировалс€ на хабе. ¬ыберите другой ник.")
	};
