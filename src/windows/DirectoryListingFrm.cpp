...
#define HGAP               8
#define BUTTON_ICON_WIDTH  36
#define BUTTON_ICON_HEIGHT 36
...
void DirectoryListingFrame::openWindow(const tstring& aFile, const UserPtr& aUser, int64_t aSpeed) {
...
LRESULT DirectoryListingFrame::OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled) {
  ...
  m_userAddToFriends.Create(IDI_USER_ADD_TO_FAVORITES, m_hWnd, NULL, IDC_ADD_TO_FAVORITES);
  m_userAddToFriends.setHintText(TSTRING(DIRECTORY_LISTING_ADD_TO_FAVORITES_HINT));
  m_sendPrivateMessage.Create(IDI_PRIVATE_CHAT_HEADER, m_hWnd, NULL, IDC_PRIVATEMESSAGE);
  m_sendPrivateMessage.setHintText(TSTRING(DIRECTORY_LISTING_PRIVATE_MESSAGE_HINT));
  m_downloadFile.Create(IDI_FILE_DOWNLOAD, m_hWnd, CTSTRING(FILELIST_DOWNLOAD_FILE), IDC_DOWNLOAD);
  m_downloadFolder.Create(IDI_FILE_DOWNLOAD, m_hWnd, CTSTRING(FILELIST_DOWNLOAD_DIR), IDC_DOWNLOADDIR);
  FlatIconButton* buttons[] = { &m_userAddToFriends, &m_sendPrivateMessage, &m_downloadFile, &m_downloadFolder };
  for (int i = 0; i < COUNTOF(buttons); ++i) {
    FlatIconButton* button = buttons[i];
    button->setHighLightColor(HOVER_COLOR);
    button->setHighLightBorder(true);
    button->SetFont(WinUtil::systemFont);
  }
	...
	priorityDirMenu.AppendMenu(MF_STRING, IDC_PRIORITY_PAUSED+PRIORITY_DIR_OFFSET, CTSTRING(PAUSED));
	priorityDirMenu.AppendMenu(MF_STRING, IDC_PRIORITY_LOWEST+PRIORITY_DIR_OFFSET, CTSTRING(LOWEST));
	priorityDirMenu.AppendMenu(MF_STRING, IDC_PRIORITY_LOW+PRIORITY_DIR_OFFSET, CTSTRING(LOW));
	priorityDirMenu.AppendMenu(MF_STRING, IDC_PRIORITY_NORMAL+PRIORITY_DIR_OFFSET, CTSTRING(NORMAL));
	priorityDirMenu.AppendMenu(MF_STRING, IDC_PRIORITY_HIGH+PRIORITY_DIR_OFFSET, CTSTRING(HIGH));
	priorityDirMenu.AppendMenu(MF_STRING, IDC_PRIORITY_HIGHEST+PRIORITY_DIR_OFFSET, CTSTRING(HIGHEST));
	...
	setWindowTitle();
        ...
        m_header.addWords(TSTRING(FILELIST_TITLE), m_headerFont, GetSysColor(COLOR_WINDOWTEXT));
        m_header.addWord(Text::toT(dl->getUser()->getFirstNick()), m_headerFont, GREEN_COLOR);

	bHandled = FALSE;
	return 1;
}

void DirectoryListingFrame::createCopyMenu(bool fileMode) {
	copyMenu.ClearMenu();
...
//-BugMaster: fix crash when subtract filelists and selected node is not root
	...
	int index = 0;
	if(d)
	  index = d->getComplete() ? WinUtil::getDirIconIndex() : WinUtil::getDirMaskedIndex();
...
LRESULT DirectoryListingFrame::onDownloadDirWithPrio(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	if(t != NULL) {
		...
		switch(wID-PRIORITY_DIR_OFFSET) {
...
LRESULT DirectoryListingFrame::onVideoDownload(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	if(ctrlList.GetSelectedCount() == 1) {
		ItemInfo* ii = (ItemInfo*)ctrlList.GetItemData(ctrlList.GetNextItem(-1, LVNI_SELECTED));
		try {
			if (ii->type == ItemInfo::FILE) {
				DirectoryListing::File *aFile = ii->file;
				string localPath = ShareManager::getInstance()->toRealPath(aFile->getTTH());
				if (!localPath.empty()) {
					WinUtil::showVideo(localPath);
				}
				else {
					QueueItem::List ql;
					QueueManager::getInstance()->getTargets(aFile->getTTH(), ql);
					if (!ql.empty()) {
						...
						WinUtil::showVideoSplash();
						QueueManager::getInstance()->add(SETTING(DOWNLOAD_DIRECTORY) + aFile->getName(), aFile->getSize(), aFile->getTTH(), dl->getUser(), QueueItem::FLAG_ONLINE_VIDEO|QueueItem::FLAG_MULTI_SOURCE);
					}
				}
			} 
		} catch(const Exception& e) {
...
LRESULT DirectoryListingFrame::onPM(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	UserPtr pUser = dl->getUser();
	if(pUser != (User*) NULL)
		PrivateFrameFactory::openWindow(pUser);
	return 0;
}
...
void DirectoryListingFrame::prepareFileMenuTop(OMenu &fileMenu, DirectoryListing::File *aFile) {
	fileMenu.AppendMenu(MF_STRING, IDC_DOWNLOAD, CTSTRING(DOWNLOAD));
	if (aFile != NULL && WinUtil::isVideo(aFile->getName())) {
		fileMenu.AppendMenu(MF_STRING, IDC_VIDEO, CTSTRING(FILELIST_FILEMENU_VIDEO));
	}
	fileMenu.AppendMenu(MF_POPUP, (UINT)(HMENU)targetMenu, CTSTRING(DOWNLOAD_TO));
	fileMenu.AppendMenu(MF_STRING, IDC_DOWNLOADDIR, CTSTRING(FILELIST_FILEMENU_DOWNLOAD_DIR));
	fileMenu.AppendMenu(MF_POPUP, (UINT_PTR)(HMENU)targetDirMenu, CTSTRING(FILELIST_FILEMENU_DOWNLOAD_DIR_TO));
...
void DirectoryListingFrame::prepareFileMenuBottom(OMenu &fileMenu) {
...
HRESULT DirectoryListingFrame::onContextMenu(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	...
	if (reinterpret_cast<HWND>(wParam) == ctrlList && ctrlList.GetSelectedCount() > 0) {
		...
		if(ctrlList.GetSelectedCount() == 1 && ii->type == ItemInfo::FILE) {
			...
			appendLastDirs(targetMenu, IDC_DOWNLOAD_TARGET, n);

			if (ShareManager::getInstance()->toRealPath(ii->file->getTTH()).empty()) {
				fileMenu.SetMenuDefaultItem(IDC_DOWNLOAD);
			}
			else {
				fileMenu.InsertMenu(0, MF_BYPOSITION | MF_STRING, IDC_OPEN_FILE, CTSTRING(OPEN));
				fileMenu.SetMenuDefaultItem(IDC_OPEN_FILE);
			}
			prepareFileMenuBottom(fileMenu);
			createCopyMenu(true);
			targetDirMenu.ClearMenu();
			//Append Favorite download dirs
			appendFavoriteDirs(targetDirMenu, IDC_DOWNLOAD_WHOLE_FAVORITE_DIRS);
			targetDirMenu.AppendMenu(MF_STRING, IDC_DOWNLOADDIRTO, CTSTRING(BROWSE));
			appendLastDirs(targetDirMenu, IDC_DOWNLOAD_TARGET_DIR);
			...
			prepareFileMenuTop(fileMenu, NULL);
			...
			fileMenu.EnableMenuItem(IDC_DOWNLOADDIR, MF_BYCOMMAND | MFS_DISABLED);
			fileMenu.EnableMenuItem((UINT)(HMENU)targetDirMenu, MF_BYCOMMAND | MFS_DISABLED);
			prepareFileMenuBottom(fileMenu);
			createCopyMenu(hasSelectedFiles());
			//Append Favorite download dirs
			appendLastDirs(targetMenu, IDC_DOWNLOAD_TARGET);
			if (ii->type == ItemInfo::DIRECTORY 
				&& ii->dir->getAdls()
				&& ii->dir->getParent() != dl->getRoot()) 
			{
				fileMenu.AppendMenu(MF_STRING, IDC_GO_TO_DIRECTORY, CTSTRING(GO_TO_DIRECTORY));
			}
		...
		prepareMenu(fileMenu, UserCommand::CONTEXT_FILELIST, ClientManager::getInstance()->getHubs(dl->getUser()->getCID()));
		fileMenu.TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON, pt.x, pt.y, m_hWnd);
		fileMenu.RemoveMenu((UINT)(HMENU) targetMenu, MF_BYCOMMAND);
		fileMenu.RemoveMenu((UINT)(HMENU) targetDirMenu, MF_BYCOMMAND);
		fileMenu.RemoveMenu((UINT)(HMENU) copyMenu, MF_BYCOMMAND);
...
bool DirectoryListingFrame::hasSelectedFiles() {
	int i = -1;
	while ((i = ctrlList.GetNextItem(i, LVNI_SELECTED)) != -1) {
		ItemInfo* ii = (ItemInfo*)ctrlList.getItemData(i);
		if (ii->type == ItemInfo::FILE) {
...
void DirectoryListingFrame::appendFavoriteDirs(OMenu& menu, int command) {
  StringPairList spl = FavoriteManager::getInstance()->getFavoriteDirs();
  if (spl.size() > 0) {
    int n = 0;
    for (StringPairIter i = spl.begin(); i != spl.end(); ++i) {
      menu.AppendMenu(MF_STRING, command + n, Text::toT(i->second).c_str());
      n++;
    }
    targetMenu.AppendMenu(MF_SEPARATOR);
  }
...
void DirectoryListingFrame::appendLastDirs(OMenu& menu, int command, int startIndex) {
  if (WinUtil::lastDirs.size() > 0) {
    targetDirMenu.AppendMenu(MF_SEPARATOR);
    int n = startIndex;
    for (TStringIter i = WinUtil::lastDirs.begin(); i != WinUtil::lastDirs.end(); ++i) {
      ++n;
      menu.AppendMenu(MF_STRING, command + n, i->c_str());
    }
  }
}
...
void DirectoryListingFrame::UpdateLayout(BOOL bResizeBars /* = TRUE */) {
	...
	UpdateBarsPosition(rect, bResizeBars);
        ...
        const int headerWidth = m_header.getWidth();
        const int headerDelta = max(0, (m_headerHeight - m_header.getHeight()) / 2);
        const int buttonY = (m_headerHeight - BUTTON_ICON_HEIGHT) / 2;
        m_userAddToFriends.MoveWindow(headerDelta + headerWidth + HGAP, buttonY, BUTTON_ICON_WIDTH, BUTTON_ICON_HEIGHT);
        m_sendPrivateMessage.MoveWindow(headerDelta + headerWidth + HGAP * 2 + BUTTON_ICON_WIDTH, buttonY, BUTTON_ICON_WIDTH, BUTTON_ICON_HEIGHT);
        const int btnDownloadFileWidth = m_downloadFile.getPreferredSize().cx;
        const int btnDownloadFolderWidth = m_downloadFolder.getPreferredSize().cx;
        m_downloadFolder.MoveWindow(rect.right - btnDownloadFolderWidth - HGAP, buttonY, btnDownloadFolderWidth, BUTTON_ICON_HEIGHT);
        m_downloadFile.MoveWindow(rect.right - btnDownloadFolderWidth - btnDownloadFileWidth - 2*HGAP, buttonY, btnDownloadFileWidth, BUTTON_ICON_HEIGHT);
...
LRESULT DirectoryListingFrame::OnPaint(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/) {
  CPaintDC dc(m_hWnd);
  CRect r;
  GetClientRect(r);
  dc.FillRect(r, GetSysColorBrush(COLOR_BTNFACE));
  dc.SetBkColor(GetSysColor(COLOR_BTNFACE));
  const int delta = max(0, (m_headerHeight - m_header.getHeight()) / 2);
  m_header.draw((HDC) dc, delta, delta);
  return 0;
}

HTREEITEM DirectoryListingFrame::findFile(const StringSearch& str, HTREEITEM root, int &foundFile, int &skipHits)
{
...
LRESULT DirectoryListingFrame::onCopy(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	while ((i = ctrlList.GetNextItem(i, LVNI_SELECTED)) != -1) {
		...
		case IDC_COPY_LINK:
			if (ii->type == ItemInfo::FILE)
				sCopy = WinUtil::getMagnet(ii->file->getTTH(), Text::toT(ii->file->getName()), ii->file->getSize());
			break;
		case IDC_COPY_TTH:
			if(ii->type == ItemInfo::FILE)
		...
		case IDC_COPY_WMLINK://[+]necros
			if (ii->type == ItemInfo::FILE)
				sCopy = WinUtil::getWebMagnet(ii->file->getTTH(), Text::toT(ii->file->getName()), ii->file->getSize());
			break;
		default:
...
LRESULT DirectoryListingFrame::onClose(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled) {
	if(loading) {
        ...
        clearList();
        frames.erase(m_hWnd);
        ctrlList.saveHeaderOrder(SettingsManager::DIRECTORLISTINGFRAME_ORDER, SettingsManager::DIRECTORLISTINGFRAME_WIDTHS, SettingsManager::DIRECTORYLISTINGFRAME_VISIBLE); // !SMT!-UI
...
void DirectoryListingFrame::getItemColor(const Flags::MaskType flags, COLORREF &fg, COLORREF &bg)
{
        ...
        if (flags & DirectoryListing::FLAG_QUEUED)
                fg = (flags & DirectoryListing::FLAG_NOT_QUEUED)?
                        WinUtil::blendColors(SETTING(SEARCH_ALTERNATE_COLOUR), SETTING(TEXT_COLOR)) :
                        SETTING(SEARCH_ALTERNATE_COLOUR);
}
...
DirectoryListingFrame::ItemInfo::ItemInfo(DirectoryListing::File* f) : type(FILE), file(f) {
        ...
        if(f->isSet(DirectoryListing::FLAG_SHARED))
                columns[COLUMN_PATH] = Text::toT(ShareManager::getInstance()->toRealPath(f->getTTH())); // !PPA!
        else if(f->isSet(DirectoryListing::FLAG_QUEUED)) { // !SMT!-UI
                StringList sl;
                 QueueManager::getInstance()->getTargets(f->getTTH(), sl);
                if (sl.size()) columns[COLUMN_PATH] = Text::toT(sl[0]);
        }
}
...
/**
 * @file
 * $Id: DirectoryListingFrm.cpp,v 1.11.2.1 2008/11/04 14:09:33 alexey Exp $
 */
