...
public:
	...
	void rebuildArrows()
	{
		...
		dc.Polyline(pathArrowLong, COUNTOF(pathArrowLong));
		dc.SelectPen(penShadow);
		dc.Polyline(pathArrowShort, COUNTOF(pathArrowShort));

		// create down arrow
		dc.SelectBitmap(downArrow);
		dc.FillRect(&rect, brush);
		for (int i=0; i < COUNTOF(pathArrowShort); ++i)
		{
		...
		dc.Polyline(pathArrowShort, COUNTOF(pathArrowShort));
		for (int i=0; i < COUNTOF(pathArrowLong); ++i)
		{
...
/**
 * @file
 * $Id: ListViewArrows.h,v 1.2 2008/03/10 07:26:54 alexey Exp $
 */
