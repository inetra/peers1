...
PropPage::TextItem AVIPreview::texts[] = {
	...
	{ IDC_CUSTOM_VIDEO_PLAYER, ResourceManager::CUSTOM_VIDEO_PLAYER },
	{ IDC_BROWSEDIR, ResourceManager::BROWSE_ACCEL },
	{ 0, ResourceManager::SETTINGS_AUTO_AWAY }
};

PropPage::Item AVIPreview::items[] = {
  { IDC_CUSTOM_VIDEO_PLAYER, SettingsManager::USE_CUSTOM_VIDEO_PLAYER, PropPage::T_BOOL },
  { IDC_VIDEO_PLAYER,	SettingsManager::CUSTOM_VIDEO_PLAYER, PropPage::T_STR }, 
  { 0, 0, PropPage::T_END }
};
...
LRESULT AVIPreview::onCustomVideoPlayer(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	updateEnablement();
	return 0;
}

void AVIPreview::updateEnablement() {
	bool custom = IsDlgButtonChecked(IDC_CUSTOM_VIDEO_PLAYER) == BST_CHECKED;
	::EnableWindow(GetDlgItem(IDC_VIDEO_PLAYER), custom);
	::EnableWindow(GetDlgItem(IDC_BROWSEDIR), custom);
}
...
LRESULT AVIPreview::onClickedBrowseDir(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	static const TCHAR types[] = _T("Executable Files (*.exe)\0*.exe\0All Files\0*.*\0");
	tstring player = GetDlgItemString(IDC_VIDEO_PLAYER);
	...
	if (WinUtil::browseFile(player, m_hWnd, false, Util::emptyStringT, types)) {
		SetDlgItemText(IDC_VIDEO_PLAYER, player.c_str());
	}
	return 0;
}
