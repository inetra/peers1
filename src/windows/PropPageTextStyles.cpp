...
// !necros!
LRESULT PropPageTextStyles::onBanColor(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
        CColorDialog dlg(SETTING(BAN_COLOR), CC_FULLOPEN);
        if(dlg.DoModal() == IDOK)
                SettingsManager::getInstance()->set(SettingsManager::BAN_COLOR, (int)dlg.GetColor());
        return 0;
}

// !necros!
LRESULT PropPageTextStyles::onDupeColor(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
        CColorDialog dlg(SETTING(DUPE_COLOR), CC_FULLOPEN);
        if(dlg.DoModal() == IDOK)
                SettingsManager::getInstance()->set(SettingsManager::DUPE_COLOR, (int)dlg.GetColor());
        return 0;
}
...
void PropPageTextStyles::write()
{
	...
	settings->set(SettingsManager::TEXT_FONT, Text::fromT(WinUtil::encodeFont(m_Font)));
