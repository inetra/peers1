...
id(0)
{
  m_hiddenWindow.m_manager = this;
  TimerManager::getInstance()->addListener(this);
  m_hiddenWindow.Create(NULL);
}
...
void PopupManager::Show(const tstring &aMsg, const tstring &aTitle, int Icon, bool preview /*= false*/) {
	...
	if (!Util::getAway() && BOOLSETTING(POPUP_AWAY) && !preview)
        ...
        if(!WinUtil::isAppMinimized() && BOOLSETTING(POPUP_MINIMIZED) && !preview) {
		return;
	}
...
void PopupManager::on(TimerManagerListener::Second /*type*/, uint32_t /*tick*/) {
	//post a message and let the main window thread take care of the window
        ::PostMessage(m_hiddenWindow, WM_SPEAKER, 0, 0);
}
