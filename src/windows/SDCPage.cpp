...
PropPage::TextItem SDCPage::texts[] = {
	...
	{ IDC_USERLIST_CHAT_DBLCLICKACTION, ResourceManager::USERLISTCHATDBLCLICKACTION },
...
#define setMinMax(x, y, z) CUpDownCtrl(GetDlgItem(x)).SetRange32(y, z);
...
LRESULT SDCPage::onInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	userlistChatAction.Attach(GetDlgItem(IDC_USERLIST_CHAT_DBLCLICK));
	...
	userlistChatAction.AddString(CTSTRING(GET_FILE_LIST));
	userlistChatAction.AddString(CTSTRING(SEND_PUBLIC_MESSAGE));
	userlistChatAction.AddString(CTSTRING(SEND_PRIVATE_MESSAGE));
	userlistChatAction.AddString(CTSTRING(MATCH_QUEUE));
	userlistChatAction.AddString(CTSTRING(GRANT_EXTRA_SLOT));
	userlistChatAction.AddString(CTSTRING(ADD_TO_FAVORITES));
	...
	userlistChatAction.SetCurSel(SETTING(USERLIST_CHAT_DBLCLICK));
...
void SDCPage::write()
{
	...
	settings->set(SettingsManager::USERLIST_CHAT_DBLCLICK, userlistChatAction.GetCurSel());
...
/**
 * @file
 * $Id: SDCPage.cpp,v 1.4 2008/03/10 06:57:18 alexey Exp $
 */
