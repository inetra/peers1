...
LRESULT Popups::onInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	PropPage::read(items);
	PropPage::readCheckBoxList(listItems, GetDlgItem(IDC_POPUPLIST));

	ctrlPopupType.Attach(GetDlgItem(IDC_POPUP_TYPE));
	ctrlPopupType.AddString(_T("Balloon popup"));
	ctrlPopupType.AddString(_T("Custom popup"));
	ctrlPopupType.AddString(_T("Splash popup"));
...
void Popups::write()
{
	...
	PropPage::writeCheckBoxList(listItems, GetDlgItem(IDC_POPUPLIST));
...
{
	PopupManager::getInstance()->Show(TSTRING(FILE) + _T(": ") _T(APPNAME) _T(".exe\n") +
...
/**
 * @file
 * $Id: Popups.cpp,v 1.7 2008/03/10 07:51:06 alexey Exp $
 */
