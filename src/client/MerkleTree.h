...
public:
	...
	{
		const size_t n = calcBlocks(aFileSize, aBlockSize);
		leaves.reserve(n);
	...
	void getLeafData(vector<uint8_t>& p_buf) //[+]PPA
	{
        const size_t l_size = getLeaves().size(); 
		p_buf.resize(l_size * HASH_SIZE);
		uint8_t* p = &p_buf[0];
...
/**
 * @file
 * $Id: MerkleTree.h,v 1.1.1.1 2007/09/27 13:21:19 alexey Exp $
 */
