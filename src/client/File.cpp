...
#ifdef _WIN32
uint32_t File::getLastModified(HANDLE file) throw() {
	FILETIME f = {0};
	::GetFileTime(file, NULL, NULL, &f);
	return convertTime(&f);
}
#endif
...
bool File::exists(const string& aFile) throw() {
  HANDLE file = ::CreateFile(Text::toT(aFile).c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL);
  if (file == INVALID_HANDLE_VALUE) {
	  DWORD error = GetLastError();
#ifdef _DEBUG
	  dcdebug("File::exists(%s)=%d\n", Text::utf8ToAcp(aFile).c_str(), error);
#endif
...
void File::renameFile(const string& source, const string& target) throw(FileException) {
	tstring src = Text::toT(source);
	tstring dst = Text::toT(target);
	if(!::MoveFile(src.c_str(), dst.c_str())) {
		if (!::CreateHardLink(Text::toT(source).c_str(), Text::toT(target).c_str(), NULL)) {
...
bool File::createHardLink(const string& source, const string& target) {
	return ::CreateHardLink(Text::toT(source).c_str(), Text::toT(target).c_str(), NULL) != FALSE;
}

void File::atomicRename(const string& source, const string& target) throw(FileException) {
	if(!::MoveFileEx(Text::toT(source).c_str(), Text::toT(target).c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH)) {
		throw FileException(Util::translateError(GetLastError()));
	}
}

void File::atomicRename(const tstring& source, const tstring& target) throw(FileException) {
	if(!::MoveFileEx(source.c_str(), target.c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH)) {
		throw FileException(Util::translateError(GetLastError()));
	}
}
...
bool File::isAbsolute(const string& path) throw() {
        return path.size() > 2 && ((path[1] == ':' && path[2] == '\\') || path[0] == '/' || (path[0] == '\\' && path[1] == '\\')); // !SMT!-F
}
