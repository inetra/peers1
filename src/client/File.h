...
//[-]PPA [Doxygen 1.5.1] #include "SettingsManager.h"

//[-]PPA [Doxygen 1.5.1]  #include "Util.h"
//[-]PPA [Doxygen 1.5.1]  #include "Text.h"
#include "Streams.h"
...
public:
...
#ifdef _WIN32
	static uint32_t getLastModified(HANDLE file) throw();
#endif

	static bool exists(const string& aFile) throw();
	...
	static bool createHardLink(const string& source, const string& target);
	static void atomicRename(const string& source, const string& target) throw(FileException);
	static void atomicRename(const tstring& source, const tstring& target) throw(FileException);
...
/**
 * @file
 * $Id: File.h,v 1.2.2.1 2008/06/06 17:25:54 alexey Exp $
 */
