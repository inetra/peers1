...
public:
	ListLoader(DirectoryListing* aList, DirectoryListing::Directory* root, 
		 bool aUpdating, const UserPtr& p_user) 
		 : list(aList), cur(root), base("/"), inListing(false), 
		 updating(aUpdating), user(p_user) { 
	}
...
void DirectoryListing::Directory::checkDupes(const DirectoryListing* lst)
{
        ...
        for(Directory::Iter i = directories.begin(); i != directories.end(); ++i) {
                (*i)->checkDupes(lst);
                result |= (*i)->getFlags() & (FLAG_SHARED | FLAG_NOT_SHARED | FLAG_QUEUED | FLAG_NOT_QUEUED);
	}
	...
	for(File::Iter i = files.begin(); i != files.end(); ++i) {
		...
		//of no interest
		if((*i)->getSize() > 0) {
                        if (ShareManager::getInstance()->isTTHShared((*i)->getTTH()))
                                (*i)->setFlag(FLAG_SHARED);
                        else
                                (*i)->setFlag(FLAG_NOT_SHARED);
                        if (lst->isTTHQueued((*i)->getTTH()))
                                (*i)->setFlag(FLAG_QUEUED);
                        else
                                (*i)->setFlag(FLAG_NOT_QUEUED);
                        result |= (*i)->getFlags() & (FLAG_SHARED | FLAG_NOT_SHARED | FLAG_QUEUED | FLAG_NOT_QUEUED);
		}
	}
        setFlags(result);
...
{
        // QueueManager::getInstance()->getTargets() is slow (linear), fill look-up
        QueueManager::getInstance()->getQueue(queued);
