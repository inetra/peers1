...
public:
	...
	void fromXML(const string& tmp) throw(SimpleXMLException);
private:
	...
	string::size_type fromXML(const string& tmp, string& data, const string& n = Util::emptyString, string::size_type start = 0, int depth = 0) throw(SimpleXMLException);
...
public:
	...
	const string& getChildData() const throw(SimpleXMLException) {
        ...
        const string& getChildData(const string& aName) throw(SimpleXMLException);
...
/**
 * @file
 * $Id: SimpleXML.h,v 1.2 2007/11/23 07:19:20 alexey Exp $
 */
