...
#endif
*/
	  vector<T>::erase(remove(begin(), end(), p_Value), end());
  }
};
...
#define GET_PROTECTED_SET(type, name, name2) \
private: type name; \
public: inline TypeTraits<type>::ParameterType get##name2() const { return name; } \
protected: inline void set##name2(TypeTraits<type>::ParameterType a##name2) { name = a##name2; }
...
public:
	...
	static string m_dot;
	static string m_dot_dot;
	static tstring m_dotT;
	static tstring m_dot_dotT;
	...
	static wstring formatBytesW(int64_t aBytes) { return formatBytesW(aBytes, false); }
	static wstring formatBytesW(int64_t aBytes, bool useIntegerNumber);
	...
	static string toString(unsigned short val) {
		char buf[8];
		sprintf_s(buf, sizeof(buf), "%u", (unsigned int)val);
		return buf;
	}
	...
	static string toString(long val) {
		char buf[32];
		sprintf_s(buf, sizeof(buf), "%ld", val);
		return buf;
	}
	...
	static string toString(long long val) {
		char buf[32];
		sprintf_s(buf, sizeof(buf), I64_FMT, val);
		return buf;
	}
	static string toString(unsigned long long val) {
		char buf[32];
		sprintf_s(buf, sizeof(buf), U64_FMT, val);
		return buf;
	}
	static string toString(double val) {
		char buf[16];
		sprintf_s(buf, sizeof(buf), "%0.2f", val);
		return buf;
	}
	...
	static string toHexEscape(char val) {
		...
		sprintf_s(buf, sizeof(buf), "%%%X", val&0x0FF);
		return buf;
	}
	static char fromHexEscape(const string aString) {
		unsigned int res = 0;
		sscanf_s(aString.c_str(), "%X", &res);
	...
	static void getLocalIp(vector<string> &addresses);
	...
	static bool startsWith(const string& s, const string& substr);

	//static string getOsVersion();

	static bool inline isVista() { return LOBYTE(LOWORD(GetVersion())) >= 6; }
	...
	static void setMinimized(bool value) { minimized = value; }
	...
	static bool readRegistryBoolean(const TCHAR* valueName, bool defaultValue);
	static bool readRegistryBoolean(const string& valueName, bool defaultValue);

	static int64_t abs_64(int64_t value) { return value < 0 ? -value : value; }
...
/**
 * @file
 * $Id: Util.h,v 1.5 2008/03/31 15:20:30 alexey Exp $
 */
