...
class SettingsManager : public ParametrizableSingleton<SettingsManager,string>, public Speaker<SettingsManagerListener>
{
public:
	...
	enum StrSetting { STR_FIRST,
		NICK = STR_FIRST, UPLOAD_SPEED, DESCRIPTION, DOWNLOAD_DIRECTORY, DOWNLOAD_DIRECTORY_SHARED,
		EMAIL, EXTERNAL_IP,
		...
		UPDATE_URL, SOUND_TTH, SOUND_EXC, SOUND_HUBCON,  SOUND_HUBDISCON, SOUND_FAVUSER, SOUND_FAVUSER_OFFLINE, SOUND_TYPING_NOTIFY,
		...
		UNUSED_URL_GET_IP, // [+]PPA
        PM_PASSWORD, PM_PASSWORD_HINT, // !SMT!-PSW
		COPY_WMLINK,//[+]necros
	...
	enum IntSetting { INT_FIRST = STR_LAST + 1,
		...
		UNUSED_DONT_ANNOUNCE_NEW_VERSIONS,
		...
		I_DOWN_SPEED, H_DOWN_SPEED, DOWN_TIME, 
		...
		DISCONNECTING_ENABLE, MIN_FILE_SIZE, UPLOADQUEUEFRAME_SHOW_TREE,
		SEGMENTS_MANUAL, NUMBER_OF_SEGMENTS, PERCENT_FAKE_SHARE_TOLERATED,
		SEND_UNKNOWN_COMMANDS, DISCONNECT,
		...
		ADD_FINISHED_INSTANTLY, AWAY, USE_CTRL_FOR_LINE_HISTORY, 
                POPUP_HUB_CONNECTED, POPUP_HUB_DISCONNECTED, POPUP_FAVORITE_CONNECTED, POPUP_FAVORITE_DISCONNECTED, POPUP_CHEATING_USER, POPUP_DOWNLOAD_START,
		...
		USERLIST_DBLCLICK, USERLIST_CHAT_DBLCLICK, TRANSFERLIST_DBLCLICK, CHAT_DBLCLICK, ADC_DEBUG,
		UNUSED_TOGGLE_ACTIVE_WINDOW, 
		...
		BOLD_HUB, BOLD_PM, BOLD_SEARCH, 
		UNUSED_TABS_ON_TOP, 
		SOCKET_IN_BUFFER, SOCKET_OUT_BUFFER,
		COLOR_RUNNING, COLOR_DOWNLOADED, COLOR_VERIFIED, COLOR_AVOIDING, AUTO_REFRESH_TIME, USE_TLS, OPEN_WAITING_USERS, 
		...
		PM_PREVIEW, FILTER_ENTER, POPUP_TIME, AWAY_TIME_THROTTLE, AWAY_START, AWAY_END, PROGRESSBAR_ODC_BUMPED, TOP_SPEED, STEALTHY_STYLE, PSR_DELAY,
		IP_IN_CHAT, COUNTRY_IN_CHAT, TOP_UP_SPEED, BROADCAST, SETTINGS_STATE, PAGE, CHATBUFFERSIZE, ENABLE_HUBTOPIC, HUB_SMALL, PG_LAST_UP, FORMAT_BIU,
		MEDIA_PLAYER, PROT_FAVS, MAX_MSG_LENGTH, POPUP_BACKCOLOR, POPUP_TEXTCOLOR, POPUP_TITLE_TEXTCOLOR, SORT_FAVUSERS_FIRST, SHOW_SHELL_MENU, OPEN_LOGS_INTERNAL,
		NO_EMOTES_LINKS,
                
		NSLOOKUP_MODE, NSLOOKUP_DELAY, // !SMT!-IP
        BAN_SLOTS, BAN_SHARE, BAN_LIMIT, BANMSG_PERIOD, BAN_STEALTH, BAN_FORCE_PM, BAN_SKIP_OPS, EXTRASLOT_TO_DL, // !SMT!-S
        BAN_COLOR, DUPE_COLOR, MULTILINE_CHAT_INPUT, EXTERNAL_PREVIEW, SEND_SLOTGRANT_MSG, FAVUSERLIST_DBLCLICK, // !SMT!-UI
        PROTECT_PRIVATE, // !SMT!-PSW

        UP_TRANSFER_COLORS, // By Drakon
		STARTUP_BACKUP, // By Drakon

		GLOBAL_HUBFRAME_CONF, USE_ANTIVIR, IGNORE_USE_REGEXP_OR_WC,
		STEALTHY_INDICATE_SPEEDS, OLD_TRAY_BEHAV, OLD_NAMING_STYLE, OLD_ICONS_MODE, COLOUR_DUPE, NO_TTH_CHEAT,
		OLD_CLIENT_RAW, DELETE_CHECKED, TOPMOST, LOCK_TOOLBARS, AUTO_COMPLETE_SEARCH, KEEP_DL_HISTORY, KEEP_UL_HISTORY,
		UNUSED_SHOW_QUICK_SEARCH, 
                SEARCH_DETECT_TTH, 
                FULL_FILELIST_NFO, 
		UNUSED_ZION_TABS, 
		NON_HUBS_FRONT, 
                UNUSED_GLOBAL_MINI_TABS, 
                BLEND_OFFLINE_SEARCH,
		MAX_RESIZE_LINES,
                USE_CUSTOM_LIST_BACKGROUND,
                SETTINGS_EXPERT_MODE,
                SEARCH_EXPAND_TOOLBAR_ON_ACTIVATE,
                SEARCH_RESTORE_CONDITION_ON_ACTIVATE,
                MENU_ADVANCED,
                PROVIDER,
                OPEN_HUB_CHAT_ON_CONNECT,				
				NICK_ADD_UNIQUE_SUFFIX,
	...
	void set(StrSetting key, string const& value) {
          string& destination = strSettings[key - STR_FIRST];
		if ( (key == NICK) && (value.size() > 35) ) {
			destination = value.substr(0, 35);
		} else if ( (key == DESCRIPTION) && (value.size() > 50) ) {
			destination = value.substr(0, 50);
		} else if ( (key == EMAIL) && (value.size() > 64) ) {
			destination = value.substr(0, 64);
                } else if (key == DOWNLOAD_DIRECTORY || key == TEMP_DOWNLOAD_DIRECTORY) {
                  destination = Util::replace(value, "\\\\", "\\");
		} else {
			destination = value;
		}
		isSet[key] = !destination.empty();
	}
	...
	void set(IntSetting key, int value) {
	  //[+]PPA
 	    if((key == BAN_SHARE) && (value < 0)) {
			value = 0;
		}
 	    if((key == BAN_SHARE) && (value > 20)) {
			value = 20;
		}
 	    if((key == BAN_LIMIT) && (value < 0)) {
			value = 0;
		}
 	    if((key == BAN_LIMIT) && (value >50)) {
			value = 50;
		}
 	    if((key == BAN_SLOTS) && (value < 0)) {
			value = 0;
		}
 	    if((key == BAN_SLOTS) && (value > 15)) {
			value = 15; 
		}
 	    if((key == SLOTS) && (value < 1)) {
			value = 1;
		}
 	    if((key == SLOTS) && (value > 500)) {
			value = 500;
		}
		...
		if((key == SET_MINISLOT_SIZE) && (value < 16)) {
			value = 16;
		}

		if((key == NUMBER_OF_SEGMENTS) && (value > 200)) {
			value = 200;
		}

		if((key == SEARCH_TIME) && (value < 2)) {
			value = 2;
		}

		if((key == MINIMUM_SEARCH_INTERVAL) && (value < 20)) {
			value = 20;
		}

		if(key == PROGRESS_BACK_COLOR) {
			colorchk = value;
		}

		if((key == COLOR_AVOIDING) && (value + colorchk > RGB(255, 255, 255))) {
			value = RGB(100, 0, 0);
		}

		if((key == PSR_DELAY) && (value < 5)) {
			value = 5;
		}
...
*/
	...
	bool isDefault(int aSet) const { return !isSet[aSet]; }

	void load() {
		load(Util::getConfigPath() + "DCPlusPlus.xml", true);
	}
	void save() {
		save(Util::getConfigPath() + "DCPlusPlus.xml");
	}

	void load(const string& aFileName, bool main);
	void save(const string& aFileName);
        void setDefaults(); // !SMT!-S

private:
	friend class ParametrizableSingleton<SettingsManager,string>;
        string m_version;
	SettingsManager(const string& version);
...
/**
 * @file
 * $Id: SettingsManager.h,v 1.15.2.3 2008/12/08 19:26:54 alexey Exp $
 */
